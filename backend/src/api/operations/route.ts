...
import { auditLogs, damageReports, housekeepingTasks, inquiries, integrationEvents, inventoryItems, maintenanceTickets, restaurantMealBookings, restaurantOrders, rooms, travelAssets, travelDiscountRequests, travelPackages, customTravelPackageItems, customTravelPackages } from '@/db/schema';
...
export async function GET(request:Request) {
  try {
    ...
    const [housekeeping,maintenance,inventory,orders,meals,damage,packages,assets,customPackages,customItems,discounts,leads,audit,integrations]=await Promise.all([
      ...
      operational?db.select().from(inventoryItems).where(eq(inventoryItems.propertyId,propertyId)).orderBy(desc(inventoryItems.updatedAt)).limit(500):[],
      operational||financial?db.select().from(restaurantOrders).where(eq(restaurantOrders.propertyId,propertyId)).orderBy(desc(restaurantOrders.createdAt)).limit(200):[],
      operational?db.select().from(restaurantMealBookings).where(eq(restaurantMealBookings.propertyId,propertyId)).orderBy(desc(restaurantMealBookings.serviceDate)).limit(200):[],
    ...
    return apiJson({housekeeping,maintenance,inventory,restaurantOrders:orders,restaurantMealBookings:meals,damageReports:damage,packages,travelAssets:assets,customPackages,customPackageItems:customItems,discountRequests:discounts,inquiries:leads,audit,integrations});
