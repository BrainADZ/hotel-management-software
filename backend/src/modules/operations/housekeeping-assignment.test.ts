...
describe('housekeeping task assignment conflicts',()=>{
  ...
  it.each(['src/services/front-desk/service.ts','src/modules/billing/service.ts'])('%s creates an unassigned checkout cleaning task',file=>{const source=readFileSync(file,'utf8');expect(source).toContain("taskType:'CHECKOUT_CLEANING'");expect(source).toContain("status:'UNASSIGNED'");expect(source).toContain('onConflictDoNothing()');});
  ...
  it('housekeeping queue is scoped by authenticated assignee',()=>{const source=readFileSync('src/api/operations/route.ts','utf8');expect(source).toContain("c.actor.role==='HOUSEKEEPING'");expect(source).toContain('eq(housekeepingTasks.assignedUserId,c.actor.id)');});
});
