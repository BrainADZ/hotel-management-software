...
const referenceSchema = z.object({
  ...
  localAmountRupees:z.number().multipleOf(0.01).nonnegative().max(20000000),
  taxRupees:z.number().multipleOf(0.01).nonnegative().max(20000000),
...
export async function reconcileReference(c:ReservationContext,raw:Record<string,unknown>) {
  ...
  return getDb().transaction(async tx=>{
    ...
    if(existing&&input&&(existing.documentHash!==input.documentHash||existing.localAmountRupees!==input.localAmountRupees||existing.bookingReference!==input.bookingReference||existing.offlineReference!==input.offlineReference))throw new DomainError('BILL_REFERENCE_CONFLICT','This bill reference already contains different data.',409);
    ...
    const matches=folio.totalRupees===(input?.localAmountRupees??existing!.localAmountRupees)&&folio.taxRupees===(input?.taxRupees??existing!.taxRupees);
    ...
    if(!existing&&input)await tx.insert(offlineBills).values({...input,propertyId:c.property.id,reservationId:reservation.id,guestId:reservation.guestId,generatedBy:c.actor.name,cloudAmountRupees:folio.totalRupees,status,notes:'Reference reconciliation. Original PDF is retained on the originating device.'});
    else await tx.update(offlineBills).set({status,cloudAmountRupees:folio.totalRupees,...(!registering?{verifiedAt:now,verifiedBy:c.actor.id}:{})}).where(and(eq(offlineBills.id,id),eq(offlineBills.propertyId,c.property.id)));
    ...
    return {id,status,cloudAmountRupees:folio.totalRupees,financialReplay:false};
  });
}
