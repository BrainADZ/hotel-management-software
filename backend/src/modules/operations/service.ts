...
import { appUsers, auditLogs, damagePolicyRules, damageReports, folioLines, folios, housekeepingTasks, inventoryItems, maintenanceTickets, roomInspections, rooms } from '@/db/schema';
...
export async function mutateOperations(c: ReservationContext, raw: Record<string, unknown>) {
  if(c.actor.organisationId!==c.property.organisationId||c.actor.propertyId!==c.property.id) throw new DomainError('PROPERTY_FORBIDDEN','An authenticated property context is required.',403);
  const action = String(raw.action ?? '');
  if (['REGISTER_OFFLINE_BILL','VERIFY_OFFLINE_BILL'].includes(action)) return reconcileReference(c,raw);
  if (isWorkflowAction(action)) return mutateWorkflow(c, action, raw);
  if (action === 'UPSERT_INVENTORY') return upsertInventory(c, raw);
  if (action === 'ASSIGN_HOUSEKEEPING_TASK') return assignHousekeepingTask(c, raw);
  if (['RECORD_HOUSEKEEPING_OUTCOME','SUBMIT_ROOM_INSPECTION'].includes(action) && !['OWNER','MANAGER','HOUSEKEEPING'].includes(c.actor.role)) throw new DomainError('FORBIDDEN','Housekeeping work is restricted to the assigned housekeeper or management.',403);
...
async function assignHousekeepingTask(c: ReservationContext, raw: Record<string, unknown>) {
  assertHousekeepingAssignmentActor(c.actor.role);
  const taskId = String(raw.taskId ?? '').trim(), assigneeId = String(raw.assigneeId ?? '').trim(), expectedVersion = integer(raw.expectedVersion, 'Expected version');
  if (!taskId || !assigneeId) throw new DomainError('INVALID_HOUSEKEEPING_ASSIGNMENT', 'Task and housekeeper are required.', 400);
  return getDb().transaction(async tx => {
    const task = (await tx.select({ task: housekeepingTasks, roomNumber: rooms.number }).from(housekeepingTasks).innerJoin(rooms, eq(rooms.id, housekeepingTasks.roomId)).where(and(eq(housekeepingTasks.id, taskId), eq(housekeepingTasks.propertyId, c.property.id))).limit(1))[0];
    if (!task) throw new DomainError('NOT_FOUND', 'Housekeeping task was not found.', 404);
    assertHousekeepingTaskAssignable(task.task, expectedVersion);
    const assignee = (await tx.select({ id: appUsers.id, organisationId: appUsers.organisationId, propertyId: appUsers.propertyId, role: appUsers.role, active: appUsers.active, name: appUsers.name, displayName: appUsers.displayName }).from(appUsers).where(eq(appUsers.id, assigneeId)).limit(1))[0];
    assertEligibleHousekeeper(assignee, c.actor.organisationId, c.property.id);
    const assigneeName = assignee.displayName ?? assignee.name, timestamp = now(), action = task.task.assignedUserId ? 'HOUSEKEEPING_TASK_REASSIGNED' : 'HOUSEKEEPING_TASK_ASSIGNED';
    const changed = await tx.update(housekeepingTasks).set({ assignedUserId: assignee.id, assignedTo: assigneeName, status: task.task.status === 'UNASSIGNED' ? 'ASSIGNED' : task.task.status, updatedAt: timestamp, version: expectedVersion + 1 }).where(and(eq(housekeepingTasks.id, taskId), eq(housekeepingTasks.propertyId, c.property.id), eq(housekeepingTasks.version, expectedVersion))).returning({ id: housekeepingTasks.id });
    if (!changed.length) throw new DomainError('STALE_HOUSEKEEPING_TASK', 'This housekeeping task changed before assignment.', 409);
    await audit(tx, c, action, 'HOUSEKEEPING_TASK', taskId, { assignedUserId: task.task.assignedUserId, assignedTo: task.task.assignedTo }, { assignedUserId: assignee.id, assignedTo: assigneeName, roomNumber: task.roomNumber, version: expectedVersion + 1 });
    return { taskId, assignedUserId: assignee.id, assignedTo: assigneeName, version: expectedVersion + 1 };
  });
}
...
async function recordOutcome(c: ReservationContext, raw: Record<string, unknown>) {
  ...
  return getDb().transaction(async tx => {
    ...
    if (c.actor.role === 'HOUSEKEEPING' && task.assignedUserId !== c.actor.id) throw new DomainError('FORBIDDEN', 'This housekeeping task is not assigned to you.', 403);
    ...
    if (outcome === 'COME_LATER' && !deferredUntil) throw new DomainError('INVALID_DEFER_TIME', 'Choose a future return time.', 400);
    ...
    if (outcome === 'DONE') {
      const [room] = await tx.select().from(rooms).where(and(eq(rooms.id,task.roomId),eq(rooms.propertyId,c.property.id))).for('update');
      if (!room || ['MAINTENANCE','OUT_OF_ORDER','OUT_OF_SERVICE'].includes(room.operationalStatus) || (task.taskType === 'CHECKOUT_CLEANING' && room.occupancyStatus !== 'VACANT')) throw new DomainError('ROOM_STATE_CHANGED','This room cannot be released by cleaning. Refresh its current status.',409);
      const pending = await tx.select({id:housekeepingTasks.id}).from(housekeepingTasks).where(and(eq(housekeepingTasks.roomId,room.id),eq(housekeepingTasks.taskType,'CHECKOUT_INSPECTION'),eq(housekeepingTasks.status,'NEEDS_INSPECTION')));
      const damage = await tx.select({id:damageReports.id}).from(damageReports).where(and(eq(damageReports.roomId,room.id),eq(damageReports.status,'PENDING_REVIEW')));
      if(pending.length||damage.length)throw new DomainError('ROOM_REVIEW_PENDING','Complete the inspection and damage review before releasing this room.',409);
      await tx.update(rooms).set({ operationalStatus: 'CLEAN', version: sql`${rooms.version}+1`, updatedAt: timestamp }).where(and(eq(rooms.id, task.roomId), eq(rooms.propertyId, c.property.id)));
    }
...
async function submitInspection(c: ReservationContext, raw: Record<string, unknown>) {
  ...
  return getDb().transaction(async tx => {
    const [key] = await tx.select({id:folios.id}).from(housekeepingTasks).innerJoin(folios,eq(folios.reservationId,housekeepingTasks.reservationId)).where(and(eq(housekeepingTasks.id,id),eq(housekeepingTasks.propertyId,c.property.id),eq(folios.propertyId,c.property.id)));
    if(!key)throw new DomainError('NOT_FOUND','Checkout inspection task not found.',404);
    await tx.execute(sql`select pg_advisory_xact_lock(hashtext(${`folio:${key.id}`}))`);
    const joined = (await tx.select({ task: housekeepingTasks, folio: folios }).from(housekeepingTasks).innerJoin(folios, eq(folios.reservationId, housekeepingTasks.reservationId)).where(and(eq(housekeepingTasks.id, id), eq(housekeepingTasks.propertyId, c.property.id))).limit(1).for('update'))[0];
    if (!joined) throw new DomainError('NOT_FOUND', 'Checkout inspection task not found.', 404);
    const { task, folio } = joined;
    if(['CLOSED','CANCELLED','CHECKED_OUT'].includes(folio.status))throw new DomainError('FOLIO_CLOSED','This folio can no longer be changed by an inspection.',409);
    if (c.actor.role === 'HOUSEKEEPING' && task.assignedUserId !== c.actor.id) throw new DomainError('FORBIDDEN', 'This housekeeping task is not assigned to you.', 403);
    ...
    if (reportId && policy) {
      ...
      if (policy.roomImpact === 'ROOM_OUT_OF_ORDER') await tx.update(rooms).set({ operationalStatus: 'MAINTENANCE', version:sql`${rooms.version}+1`, updatedAt: timestamp }).where(eq(rooms.id, task.roomId));
...
async function resolveDamage(c: ReservationContext, raw: Record<string, unknown>) {
  ...
  return getDb().transaction(async tx => {
    const [key]=await tx.select({id:damageReports.folioId}).from(damageReports).where(and(eq(damageReports.id,id),eq(damageReports.propertyId,c.property.id)));
    if(!key)throw new DomainError('NOT_FOUND','Damage report not found.',404);
    await tx.execute(sql`select pg_advisory_xact_lock(hashtext(${`folio:${key.id}`}))`);
    const row = (await tx.select({ report: damageReports, folio: folios }).from(damageReports).innerJoin(folios, eq(folios.id, damageReports.folioId)).where(and(eq(damageReports.id, id), eq(damageReports.propertyId, c.property.id))).limit(1).for('update'))[0];
    if (!row) throw new DomainError('NOT_FOUND', 'Damage report not found.', 404);
    if(['CLOSED','CANCELLED','CHECKED_OUT'].includes(row.folio.status))throw new DomainError('FOLIO_CLOSED','This folio is closed. Use the billing correction workflow.',409);
...
async function upsertInventory(c: ReservationContext, raw: Record<string, unknown>) {
  ...
  const existing = id ? (await db.select().from(inventoryItems).where(and(eq(inventoryItems.id, id), eq(inventoryItems.propertyId, c.property.id),c.actor.role==='RESTAURANT'?eq(inventoryItems.department,'RESTAURANT'):undefined)).for('update'))[0] : null;
  ...
  if(delta!==0) await db.insert(inventoryMovements).values({id:crypto.randomUUID(),propertyId:c.property.id,itemId,delta,balance:currentQuantity,reason:existing?'Inventory count adjustment':'Opening stock',actorId:c.actor.id,actorName:c.actor.name,createdAt:timestamp});
