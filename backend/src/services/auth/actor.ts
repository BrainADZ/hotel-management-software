import { DomainError, businessUnitsForRole, isAppRole } from '@hotel/shared/domain';
import { appMode } from '../app-mode';
import { resolveHostingIdentity } from './session';
import { resolveLocalSession } from './local-session';
...
export async function resolveProductionContext(request: Request, identity: HostingIdentity, repository: ContextRepository): Promise<ApplicationContext> {
  ...
  const avatar = user.customAvatarKey ? '/api/profile/avatar/file' : user.googleAvatarUrl ?? null;
  const actor: AppActor = { id: user.id, name, email: user.email, role: user.role,
  ...
  return { user: { id: actor.id, name: actor.name, email: actor.email, role: actor.role, avatar, provider: user.authProvider ?? identity.provider },
...
export async function requireApplicationContext(request: Request): Promise<ApplicationContext> {
  ...
  const identity = await resolveLocalSession(request.headers) ?? await resolveHostingIdentity(request.headers);
