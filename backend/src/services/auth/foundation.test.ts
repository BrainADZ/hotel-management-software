...
import { GET as contextGET } from '@/api/context/route';
import { GET as propertiesGET } from '@/api/properties/route';
...
describe('production authentication and context API', () => {
  ...
  it('takes role, name, email and tenant from the database, not browser claims', async () => {
    ...
    expect(actor).toEqual({ id: 'user-a', name: 'Server Name', email: 'stored@example.test', role: 'MANAGER', avatar: null, provider: 'trusted-hosting', organisationId: 'org-a', propertyId: 'p1' });
  ...
  it('returns only public context fields and private no-store response headers', async () => {
    ...
    expect(Object.keys(body.user).sort()).toEqual(['avatar', 'email', 'id', 'name', 'provider', 'role']);
