...
export const contextRepository: ContextRepository = {
  async findUser(identity) {
    ...
    const [user] = await getDb().select({ id: appUsers.id, organisationId: appUsers.organisationId,
      ...
      displayName: appUsers.displayName, googleAvatarUrl: appUsers.googleAvatarUrl, customAvatarKey: appUsers.customAvatarKey,
      authProvider: appUsers.authProvider, role: appUsers.role, active: appUsers.active }).from(appUsers)
