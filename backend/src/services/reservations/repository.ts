...
function propertyDocumentCode(value: string | null | undefined) {
...
function rowSelection() {
  return {
    id: reservations.id,
    organisationId: reservations.organisationId,
    propertyId: reservations.propertyId,
    reference: reservations.reference,
    guestId: reservations.guestId,
    primaryGuestName: reservations.primaryGuestName,
    ...
    noShowAt: reservations.noShowAt,
    noShowBy: reservations.noShowBy,
    createdBy: reservations.createdBy,
    updatedBy: reservations.updatedBy,
...
function scoped(context: ReservationContext) {
  return and(
    eq(
  ...
  const repository: ReservationRepository = {
    async atomic(key, work) {
      return root.transaction(async (transaction) => {
        ...
        return work(
          createRepository(
            transaction as unknown as ReservationDb,
            root,
          ),
        );
      });
    ...
    async list(context, query) {
      ...
      if (query.status) {
        filters.push(
          eq(reservations.status, query.status),
        );
      }
      ...
      if (query.source) {
        filters.push(
          eq(reservations.source, query.source),
        );
      }
    ...
    async findRoom(context, id) {
      ...
      return (
        (room as RoomRecord | undefined) ?? null
      );
    },
    ...
    async conflicts(
      ...
      if (excludeId) {
        filters.push(
          ne(reservations.id, excludeId),
        );
      }

      const rowsFound = await database
        .select(rowSelection())
        .from(reservations)
        .innerJoin(
          guests,
    ...
    async nextReference(context) {
      ...
      const [sequence] = await database
        ...
        .returning({
          nextValue: financialSequences.nextValue,
        });

      const number = Math.max(
        1,
        (sequence?.nextValue ?? 2) - 1,
      );

      const propertyCode =
        propertyDocumentCode(
          context.property.code,
        );

      return `${propertyCode}-BKG/${financialYear}/${String(number).padStart(6, '0')}`;
    },
    ...
    async create(context, input) {
      ...
      if (!result) {
        throw new Error(
          'Reservation insert did not return a record.',
        );
      }
    ...
    async update(
      ...
      const reservationChanges: ReservationChanges = {
        ...changes,
      };

      delete reservationChanges.guestName;
      delete reservationChanges.email;
      delete reservationChanges.phone;
      ...
      if (
        ...
        throw new DomainError(
          'CONCURRENT_RESERVATION_UPDATE',
          'The reservation changed while you were editing it. Reload and try again.',
          409,
        );
      }
    ...
    async history(context, reservationId) {
      const found = await database
        .select({
          ...
          eventType:
            reservationEvents.eventType,
          previousStatus:
            reservationEvents.previousStatus,
          newStatus:
            reservationEvents.newStatus,
          performedBy:
            reservationEvents.performedBy,
          metadata:
            reservationEvents.metadata,
          createdAt:
            reservationEvents.createdAt,
        })
        .from(reservationEvents)
        .where(
          and(
            eq(
              reservationEvents.organisationId,
              context.actor.organisationId,
            ),
            eq(
              reservationEvents.propertyId,
              context.property.id,
            ),
            eq(
        ...
        .orderBy(
          ...
          asc(reservationEvents.id),
        );

      return found.map((event) => ({
        ...event,
        eventType:
          event.eventType as ReservationEventRecord['eventType'],
        previousStatus:
          event.previousStatus as ReservationStatus | null,
        newStatus:
          event.newStatus as ReservationStatus | null,
        metadata:
          event.metadata ?? null,
      }));
    },
