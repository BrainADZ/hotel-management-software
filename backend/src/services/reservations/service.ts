import { DomainError, assertRoleCan, calculateStayNights, roleCan, validateStayDates } from '@hotel/shared/domain';
...
export class ReservationService {
  ...
  async create(context: ReservationContext, input: ReservationCreate) {
    ...
    return this.repository.atomic(`${context.property.id}:${input.roomId}`, async (repository) => {
      ...
      const reservation = await repository.create(context, { ...input, id: crypto.randomUUID(), reference, estimatedTotalRupees: total(input.nightlyRateRupees, input.taxRateBps, input.arrivalDate, input.departureDate), now });
  ...
  async edit(context: ReservationContext, id: string, input: ReservationEdit) {
    ...
    if ((input.nightlyRateRupees !== undefined || input.taxRateBps !== undefined) && !roleCan(context.actor.role, 'reservation.override')) {
    ...
    const changes: ReservationChanges = { ...input, estimatedTotalRupees: total(input.nightlyRateRupees ?? current.nightlyRateRupees, input.taxRateBps ?? current.taxRateBps, current.arrivalDate, current.departureDate) };
  ...
  private async performAction(scoped: ReservationService, repository: ReservationRepository, context: ReservationContext, current: ReservationRecord, action: ReservationAction) {
    ...
    if (action.type === 'CHANGE_DATES' || action.type === 'EXTEND_STAY' || action.type === 'SHORTEN_STAY') {
      ...
      changes = { arrivalDate: arrival, departureDate: departure, estimatedTotalRupees: total(current.nightlyRateRupees, current.taxRateBps, arrival, departure) };
      ...
      if (action.nightlyRateRupees !== undefined && !roleCan(context.actor.role, 'reservation.override')) throw new DomainError('RATE_OVERRIDE_FORBIDDEN', 'Only an owner or manager can change an agreed rate.', 403);
      ...
      const rate = action.nightlyRateRupees ?? current.nightlyRateRupees; changes = { roomId: room.id, roomType: room.roomType, nightlyRateRupees: rate, estimatedTotalRupees: total(rate, current.taxRateBps, current.arrivalDate, current.departureDate) };
      event = action.type === 'UPGRADE_ROOM' ? 'ROOM_UPGRADED' : 'ROOM_CHANGED'; metadata = { oldRoomId: current.roomId, newRoomId: room.id, oldRoomType: current.roomType, newRoomType: room.roomType, oldNightlyRateRupees: current.nightlyRateRupees, newNightlyRateRupees: rate };
