import type { AppActor, PropertyContext } from '@/services/auth/types';
...
export type RoomRecord = { id: string; propertyId: string; number: string; roomType: string; baseRateRupees: number; active: boolean; operationalStatus: string };
export type ReservationRecord = {
  ...
  sourceReference: string | null; nightlyRateRupees: number; taxRateBps: number; estimatedTotalRupees: number;
...
export type ReservationCreate = { guestName: string; email?: string; phone?: string; roomId: string; roomType: string;
  ...
  source: ReservationSource; sourceReference?: string; nightlyRateRupees: number; taxRateBps: number; specialRequests?: string; internalNotes?: string; holdUntil?: string };
export type ReservationEdit = Partial<Pick<ReservationCreate, 'guestName' | 'email' | 'phone' | 'adults' | 'children' | 'source' | 'sourceReference' | 'nightlyRateRupees' | 'taxRateBps' | 'specialRequests' | 'internalNotes'>>;
export type ReservationAction =
  | { type: 'CHANGE_DATES'; arrivalDate: string; departureDate: string }
  | { type: 'EXTEND_STAY' | 'SHORTEN_STAY'; departureDate: string }
  | { type: 'CHANGE_ROOM' | 'UPGRADE_ROOM'; roomId: string; roomType: string; nightlyRateRupees?: number }
...
export type ReservationRepository = {
  ...
  create(context: ReservationContext, input: ReservationCreate & { id: string; reference: string; estimatedTotalRupees: number; now: string }): Promise<ReservationRecord>;
