...
const catalogSchema=z.discriminatedUnion('action',[
 z.object({action:z.literal('CREATE_TRAVEL_PRODUCT'),name,durationDays:z.number().int().min(1).max(365),locations:name,capacity:z.number().int().min(1).max(1000),sellingPricePaise:z.number().int().positive().max(100_000_000)}),
 z.object({action:z.literal('SAVE_TRAVEL_ASSET'),id:z.string().optional(),expectedUpdatedAt:z.string().optional(),name,category:z.enum(['HOTEL','TRANSPORT','ACTIVITY','MEAL','GUIDE','OTHER']),description:name,pricingUnit:z.enum(['PER_PERSON','PER_NIGHT','PER_DAY','PER_TRIP']),unitPricePaise:z.number().int().nonnegative().max(100_000_000),active:z.boolean()}),
]);
export async function saveTravelCatalog(c:TravelContext,raw:unknown){
 ...
 return getDb().transaction(async tx=>{
  if(input.action==='CREATE_TRAVEL_PRODUCT'){
   ...
   const fields={name:input.name,category:input.category,description:input.description,pricingUnit:input.pricingUnit,unitPricePaise:input.unitPricePaise,active:input.active,updatedAt:now};
