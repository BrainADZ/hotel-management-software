...
export const createPackageSchema = z.object({
  ...
  basePricePaise: paise.optional(), floorPricePaise: paise.optional(), quotedPricePaise: paise.positive(),
  discountReason: z.string().trim().max(1000).optional(),
});
export const packagePricingSchema = z.object({ basePricePaise: paise, floorPricePaise: paise, expectedVersion: z.number().int().positive().optional() });
