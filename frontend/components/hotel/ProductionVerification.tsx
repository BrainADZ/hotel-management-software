...
export function ProductionVerification({state,command,refresh,notify,setView}:PlatformViewProps){
 ...
 const register=async(values:Row)=>{
  ...
  await command({action:'REGISTER_OFFLINE_BILL',id:selected.id,offlineReference:selected.offlineReference,bookingReference:values.bookingReference,deviceId:selected.deviceId,localAmountRupees:selected.totalRupees,taxRupees:selected.taxRupees,documentHash:hash,generatedAt:selected.generatedAt});
