export type ProductionCommandRoute = {
  path: string;
  method: "POST" | "PATCH";
...
function requiredId(
  ...
  if (
    ...
    throw new Error(
      `Production command requires ${field}.`,
    );
  }
...
export function routeProductionCommand(
  payload: Record<string, unknown>,
): ProductionCommandRoute {
  const action = String(
    payload.action ?? "",
  ).trim();
  ...
  if (action === "EDIT_RESERVATION") {
    return {
      path: `/api/reservations/${requiredId(
        payload,
        "reservationId",
      )}`,
      method: "PATCH",
  ...
  /*
   * IMPORTANT:
   * A checked-in guest room move is NOT a normal
   * reservation CHANGE_ROOM operation.
   *
   * It must update:
   * - active stay
   * - reservation
   * - old room status
   * - destination room status
   * - housekeeping task
   */
  if (action === "MOVE_ROOM") {
    return {
  ...
  if (operationsActions.has(action)) {
    return {
      path: "/api/operations",
      method: "POST",
      body: payload,
  ...
  if (action === "CREATE_CUSTOM_PACKAGE") {
    return {
  ...
  if (action === "SET_PACKAGE_PRICING") {
    return {
      path: `/api/travel/packages/${requiredId(
        payload,
        "packageId",
      )}/pricing`,
      method: "PATCH",
  ...
  if (
    ...
    return {
      path: `/api/travel/discount-requests/${requiredId(
        payload,
        "requestId",
      )}/decision`,
      method: "POST",
  ...
  if (
    ...
    return {
      path: `/api/travel/inquiries/${requiredId(
        payload,
        "inquiryId",
      )}`,
      method: "PATCH",
  ...
  if (
    ...
    return {
      path: `/api/travel/follow-ups/${requiredId(
        payload,
        "followUpId",
      )}`,
      method: "PATCH",
  ...
  if (reservationActions.has(action)) {
    const reservationId = requiredId(
      payload,
      "reservationId",
    );

    return {
      path: `/api/reservations/${reservationId}/actions`,
      method: "POST",
      body: {
        ...payload,
        action: undefined,
  ...
  throw new Error(
    `Unsupported production command: ${
