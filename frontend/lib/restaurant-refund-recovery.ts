...
export type PendingRestaurantRefund = { paymentId: string; amountRupees: number; reason: string; reference?: string; idempotencyKey: string };
...
export function loadRestaurantRefund(store: Store, scope: Scope): PendingRestaurantRefund | null {
  ...
  try {
    ...
    if (!value || typeof value.paymentId !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9_-]{0,99}$/.test(value.paymentId) ||
        !Number.isSafeInteger(value.amountRupees) || value.amountRupees <= 0 || value.amountRupees > 10_000_000_000 ||
        ...
        Object.keys(value).some(field => !['paymentId', 'amountRupees', 'reason', 'reference', 'idempotencyKey'].includes(field))) throw new Error();
    return value;
