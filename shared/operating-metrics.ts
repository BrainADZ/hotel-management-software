...
export function operatingMetrics(rooms:Row[],reservations:Row[],folios:Row[],operations:Row,today:string){
 ...
 const nightly=current.reduce((s,r)=>s+Number(r.nightlyRatePaise??0),0);
 ...
 inHouseGuests:current.reduce((s,r)=>s+Number(r.adults??1)+Number(r.children??0),0),pendingPayments:folios.filter(f=>Number(f.outstandingPaise)>0).length,
 revenuePaise:folios.reduce((s,f)=>s+Number(f.totalPaise??0),0),adrPaise:current.length?Math.round(nightly/current.length):0,revParPaise:rooms.length?Math.round(nightly/rooms.length):0,
